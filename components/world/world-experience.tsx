  ...
  return (
    <div className="world-screen">
      ...
      <header className="world-header">
        <Link className="world-home" href="/"><ArrowLeft size={18} /><span>Ashwin Prakash<small>Résumé</small></span></Link>
      ...
      <nav className="world-toolbar" aria-label="Explore the world">
        ...
        <label className="world-directory"><span className="sr-only">Jump to any project or experience</span><select value={pinnedId ?? ""} onChange={(event) => { if (event.target.value) { setPaused(false); setTourIndex(null); select(event.target.value) } }}><option value="">Jump to a board…</option>{(["experience", "project", "about", "contact", "resume"] as const).map((kind) => <optgroup label={kind === "project" ? "Projects" : kind[0].toUpperCase() + kind.slice(1)} key={kind}>{SIGNS.filter((item) => item.kind === kind).map((item) => <option value={item.id} key={item.id}>{item.title}</option>)}</optgroup>)}</select></label>
      ...
      {paused && <div className="world-paused"><p>Paused.</p><button onClick={() => setPaused(false)}><Play size={17} /> Resume</button></div>}
      ...
      <Dialog.Root open={help} onOpenChange={setHelp}><Dialog.Portal><Dialog.Overlay className="dialog-overlay" /><Dialog.Content className="project-dialog world-help"><Dialog.Close className="icon-button dialog-close" aria-label="Close help"><X size={21} /></Dialog.Close><p className="eyebrow">CONTROLS</p><Dialog.Title>How to get around</Dialog.Title><Dialog.Description>Walk the robot up to a board to read it.</Dialog.Description><ul><li>WASD or arrow keys to walk. Hold Shift to run.</li><li>Click a board, or press E when you&apos;re near one.</li><li>On touch screens, drag the joystick to move.</li><li>Use “Jump to a board” for direct access, or take the guided tour.</li><li>Escape closes a board. Battery saver lowers the graphics workload.</li></ul><div className="dialog-actions"><Dialog.Close className="action action-primary">Got it <ArrowRight size={17} /></Dialog.Close><Link className="action action-outline" href="/cv">Read portfolio</Link><a className="action action-outline" href="/resume.pdf">Résumé PDF</a></div></Dialog.Content></Dialog.Portal></Dialog.Root>
    </div>
  )
}
